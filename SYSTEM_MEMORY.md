...
- **`lib/error-codes.ts`** is the single registry for every "coded" API error — a Gemini/Google-style short numeric code (`#300`, `#406`, ...) attached to error responses that a user genuinely can't self-diagnose from the message alone. Grouped by kind, not HTTP status: `200-299` Auth, `300-399` Not Found, `400-499` Conflict, `500-599` Rate Limit, `900-999` Internal (30 entries total as of 2026-07-21). Each entry has `code`, `title`, `defaultMessage`, `meaning` (short, shown in the toast popup card), and `causes` (a dev-facing bullet list of likely root causes/what to check, shown only on `/help/errors`, collapsed behind a disclosure). Codes are append-only — never renumber an existing one, even to keep the file alphabetical, since a bookmark or a kitchen worker's memory could reference the old number.
