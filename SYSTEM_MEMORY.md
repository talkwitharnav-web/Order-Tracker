...
## 12. Local tooling: `startup` and `export` commands (2026-07-05)
Two new entry points, each runnable as `.\startup` / `.\export` from **either** `Restaurant/` or `Restaurant/app/` — thin `.ps1`/`.cmd` wrappers in both locations call the real logic in `scripts/`, so there's one copy of each to maintain, not two that can drift.
...
- **`scripts/docker-export.ps1`** + **`scripts/export-templates/`**: builds `app/Dockerfile` into an image (mirrors local dev — `npm run build` then `node server.js`, never `next start`), bundles it with the `postgres:16` image, a compose file (`docker-compose.export.yml`), and `run.cmd`/`run.sh` one-click launchers into `restaurant-app-export.zip` at the repo root. **Both Docker images ship inside the zip** so a target machine needs zero internet access — Docker would otherwise silently try to pull `postgres:16` from Docker Hub on first run on a machine that's never used it. See CLAUDE.md §9 for the full self-sufficiency breakdown and the real PowerShell-5.1-incompatible-RNG bug this tooling shipped with and fixed during testing.
