...
- **Pickup-window presets are 1/2/4 hours plus a Custom HH:MM entry** (changed 2026-07-11 from the original 1/6/12/24h set — a literal replacement, not additive, per explicit user correction). `Dashboard.tsx`'s `CompleteCapSettingCard` gained two plain number inputs (H, MM) behind a "Custom" button, converted to decimal hours and validated against the existing route's own `MIN_HOURS=0.1`/`MAX_HOURS=168` client-side before `PUT .../settings` — no backend change, that route already accepted any value in range.
