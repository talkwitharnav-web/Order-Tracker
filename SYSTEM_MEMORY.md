...
- `next.config.ts`: `allowedDevOrigins: ["<LAN-IP>"]` — was the actual fix for "blank page on LAN IP" (Next's dev HMR socket only trusts `localhost` by default; failed HMR connection silently prevented the real page from ever mounting). **Requires a literal IP/hostname string, does NOT support CIDR notation** (silently accepted by the schema, has no effect) — confirmed via Next's own bundled docs. If the LAN IP changes, this value goes stale and the bug recurs.
