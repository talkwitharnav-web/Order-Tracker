...
- `/help/errors` (`app/help/errors/page.tsx`, client component, NOT in `server.js`'s `PUBLIC_ALLOWED_PREFIXES` — staff-only reference, gated like `/admin/db`) is a full redesign as of 2026-07-21: compact hero (chef mascot beside the heading/search box, not stacked above — cut hero height roughly in half after user feedback that the original was "wayyy too big") with `ChefMascot` + live search (filters by code number/title/message text), a sticky sidebar category nav (`lib/error-codes.ts`'s `CATEGORIES` — slug/label/blurb per range), and one card per error with the summary always visible and a `ChevronRight`-icon disclosure toggle revealing the `causes` bullet list. `SettingsToggles` (rendered plain, no `health`/`showClock` props) sits at the top for theme/accessibility consistency with the rest of the app. The original v1 of this page (server component, flat list, no search/sidebar/mascot/causes) was explicitly rejected by the user as "too first-iteration" — do not regress toward that flatter shape.
- **Floating "glass" sticky search bar** (added 2026-07-21): once the hero's own search input scrolls out of view (tracked via `IntersectionObserver` on the hero box, not a scrollY threshold — stays correct regardless of hero height/viewport/future copy changes), a second search input pops in at `fixed top-0`, sharing the same `query` state so typing in either filters identically. Driven by an explicit 4-phase state machine (`"hidden" | "entering" | "visible" | "exiting"`, mirroring `ChefMascot`'s own swap-`Phase` pattern) rather than a single boolean — the element must stay mounted through its own ~260ms exit animation before removal, or scrolling back up makes it vanish instantly with no fade-out at all (a real bug caught live 2026-07-21). Pops in/out with this app's signature springy overshoot easing (`cubic-bezier(0.34, 1.56, 0.64, 1)`, same curve as `notification-pop-in`/`step-advance-bounce`) via `sticky-search-pop-in`/`-out` keyframes in `globals.css`, not a flat slide — automatically covered by the existing blanket Reduce Motion override, no extra work needed. Background is `bg-[var(--color-surface-1)]/25 backdrop-blur-xl` — deliberately heavy blur over a MOSTLY transparent (not solid) surface; the user's own framing (2026-07-21): translucency is a psychological signal that "the element behind this still exists, you don't have to work to remember that," so err toward more see-through, not less, on any future overlay in this app. This floating bar does NOT reserve `--reserved-top-right-w` clearance for `SettingsToggles` — it's a distinct row below the toolbar, not sharing its horizontal space; an earlier version added the clearance "to be safe" and it just shifted the centered search box ~295px off-center for no reason. If a future overlay DOES sit in the same row as `SettingsToggles`, reserve clearance on the outermost full-bleed element, never on an inner `mx-auto` centering wrapper (asymmetric padding on a centering wrapper breaks centering).
