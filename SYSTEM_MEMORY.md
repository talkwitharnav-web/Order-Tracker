...
**`/api/dev/db`'s default (unfiltered) response caps `orders` and `deletedOrders` at the 500 most-recent-by-id/deleted_at rows each** — a deliberate guard against shipping a kitchen's entire history on every page load, but it means the default Orders table view on `/admin/db` can silently exclude any order older than the current top-500 system-wide, with no pagination UI to reach further. This held up fine until a stress test (2026-07-10, ~1300 test orders) actually pushed live order count past 500 for the first time — a real kitchen's ("asdf") only order aged out of the window, and BOTH the restaurant filter dropdown (built from the capped `orders`/`deletedOrders` arrays, not the separate uncapped `restaurants` list) and the order search box (only ever filtered the already-capped in-memory rows) went blind to it, even though it was completely live. Fixed: the dropdown now sources from `restaurants` directly; `GET /api/dev/db?orderSearch=&restaurantNames=` runs a real targeted Postgres query (still capped at 500 *matching* rows, not 500 *most recent overall*) when either param is present, and the client debounces (300ms) a refetch with these whenever the search box or restaurant filter changes — every other refetch path (WS live update, modal-close, rename/password-reset) now also replays whatever search/filter is currently active via a `searchParamsRef`, instead of silently reverting to the unfiltered default. The underlying 500-row ceiling on the default/broad-search view is still architectural, not fully resolved — true pagination would be the next step if this recurs.
...
| `/api/dev/db`, `/api/dev/seed` | Admin DB view/Purge and destructive Seed. `GET` accepts optional `orderSearch`/`restaurantNames` (comma-separated) — see "500-row cap" below |
