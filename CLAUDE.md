...
- **Theme**: "warm bistro," light (cream/parchment/terracotta) + dark (espresso/terracotta), toggle in `ThemeToggle.tsx`, `data-theme` on `<html>`, no-flash inline script in `layout.tsx`. **Theme transition (2026-07-08)**: uses `document.startViewTransition()` for a random directional/circular reveal (9 variants: 4 wipes, 2 diagonals, 3 circle-expansions — see `ThemeToggle.tsx`'s `buildClipPaths()`). Falls back to instant swap if unsupported or Reduce Motion is active.
